...
export const MatchCard = ({ project, index, totalProjects = 3 }) => {
    ...
    // Deck breadth offset: Card 1 is broadest, subsequent cards inset
    const widthOffset = (totalProjects - 1 - index) * 40;
    const cardWidth = widthOffset > 0 ? `calc(100% - ${widthOffset}px)` : '100%';

    // Scroll progress for dynamic scale expansion
    ...
    return (
        <>
            ...
            >
                ...
                {project.image ? (
                    <img 
                        src={project.image} 
                        alt={project.title} 
                        className="chess-project-preview-img"
                        loading="lazy"
                    />
                ) : (
                    <div className="chess-project-preview-placeholder">
                        <Code2 size={36} className="chess-placeholder-icon" />
                        <span className="chess-placeholder-title">{project.title.split('–')[0].trim()}</span>
                    </div>
                )}

                {/* Bottom Tagline Bar */}
                {project.tagline && (
                    <div className="chess-project-preview-tagline-bar">
            ...
            <div className="chess-project-content-container">
                {/* Header Row: Live Status + Tech Stack Pills */}
                <div className="chess-project-badges-row">
                    <div className="chess-project-status-pill">
                        <Trophy size={12} className="trophy-icon" />
                        <span>VICTORY</span>
                    </div>

                    <div className="chess-project-stack-pills">
                        {project.stack && project.stack.map((tech, i) => (
                            <span key={i} className="chess-tech-pill">{tech}</span>
                        ))}
                    </div>
                </div>

                {/* Main Project Title */}
                <h3 className="chess-project-title">{project.title}</h3>

                {/* Points / Description Paragraph */}
                <div className="chess-project-description">
                    {project.points && project.points.length > 0 ? (
                        <ul className="chess-project-points-list">
                ...
                {project.tags && project.tags.length > 0 && (
                    <div className="chess-project-hashtags-row">
                        {project.tags.map((tag, i) => (
                            <span key={i} className="chess-hashtag-pill">
                ...
                {/* Action CTA Buttons */}
                <div className="chess-project-actions-row">
                    {project.links?.live && (
                        <a 
